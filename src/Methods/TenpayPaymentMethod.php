...
class TenpayPaymentMethod extends AbstractPaymentMethod
{
    ...
    {
        $title = $this->configRepo->get('wallee.tenpay_title');
        if (! empty($title)) {
            return $title;
        } else {
            return 'Tenpay';
    ...
    {
        $fee = $this->configRepo->get('wallee.tenpay_fee');
        if (! empty($fee)) {
            return (float) $fee;
        } else {
            return 0.00;
    ...
    {
        $title = $this->configRepo->get('wallee.tenpay_description');
        if (! empty($title)) {
            return $title;
        } else {
            return '';
    ...
    {
        $iconUrl = $this->configRepo->get('wallee.tenpay_icon_url');
        if (!empty($iconUrl)) {
            return $iconUrl;
        } else {
