...
class SofortBankingPaymentMethod extends AbstractPaymentMethod
{
    ...
    {
        if ($this->configRepo->get('wallee.sofortbanking_active') === "true") {
            return true;
        } else {
            return false;
        }
    ...
    public function getName(): string
    {
        ...
        $title = $translator->trans('wallee::Payment.SofortBankingTitle');
        if (! empty($title)) {
            return $title;
        } else {
    ...
    {
        $fee = $this->configRepo->get('wallee.sofortbanking_fee');
        if (! empty($fee)) {
            return (float) $fee;
        } else {
            return 0.00;
    ...
    public function getDescription(): string
    {
        ...
        $title = $translator->trans('wallee::Payment.SofortBankingDescription');
        if (! empty($title)) {
            return $title;
        } else {
            return '';
    ...
    public function getIcon(): string
    {
        ...
        $iconUrl = $translator->trans('wallee::Payment.SofortBankingIconUrl');
        if (!empty($iconUrl)) {
            return $iconUrl;
        } else {
