...
class PayboxPaymentMethod extends AbstractPaymentMethod
{
    ...
    {
        if ($this->configRepo->get('wallee.paybox_active') === "true") {
            return true;
        } else {
            return false;
        }
    ...
    public function getName(): string
    {
        ...
        $title = $translator->trans('wallee::Payment.PayboxTitle');
        if (! empty($title)) {
            return $title;
        } else {
            return 'paybox';
    ...
    {
        $fee = $this->configRepo->get('wallee.paybox_fee');
        if (! empty($fee)) {
            return (float) $fee;
        } else {
            return 0.00;
    ...
    public function getDescription(): string
    {
        ...
        $title = $translator->trans('wallee::Payment.PayboxDescription');
        if (! empty($title)) {
            return $title;
        } else {
            return '';
    ...
    public function getIcon(): string
    {
        ...
        $iconUrl = $translator->trans('wallee::Payment.PayboxIconUrl');
        if (!empty($iconUrl)) {
            return $iconUrl;
        } else {
