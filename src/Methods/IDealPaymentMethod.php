...
class IDealPaymentMethod extends AbstractPaymentMethod
{
    ...
    {
        if ($this->configRepo->get('wallee.ideal_active') === "true") {
            return true;
        } else {
            return false;
        }
    ...
    public function getName(): string
    {
        ...
        $title = $translator->trans('wallee::Payment.IDealTitle');
        if (! empty($title)) {
            return $title;
        } else {
            return 'iDeal';
    ...
    {
        $fee = $this->configRepo->get('wallee.ideal_fee');
        if (! empty($fee)) {
            return (float) $fee;
        } else {
            return 0.00;
    ...
    public function getDescription(): string
    {
        ...
        $title = $translator->trans('wallee::Payment.IDealDescription');
        if (! empty($title)) {
            return $title;
        } else {
            return '';
    ...
    public function getIcon(): string
    {
        ...
        $iconUrl = $translator->trans('wallee::Payment.IDealIconUrl');
        if (!empty($iconUrl)) {
            return $iconUrl;
        } else {
